...
import { createReferralSchema, referralRespondSchema } from '../middleware/validationSchemas';
...
router.post('/', authenticate, validate(createReferralSchema), async (req: AuthenticatedRequest, res, next) => {
  try {
    const db = createUserScopedClient(req.token!);
...
router.put('/:id/respond', authenticate, validate(referralRespondSchema), async (req: AuthenticatedRequest, res, next) => {
  try {
    const db = createUserScopedClient(req.token!);
