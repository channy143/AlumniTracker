...
router.get('/stats', authenticate, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const { data: allProfiles } = await db.from('profiles').select('id, user_id, employment_status');
...
router.get('/programs', authenticate, async (req: AuthenticatedRequest, res, next) => {
  try {
    const db = createUserScopedClient(req.token!);
    const { data: education } = await db.from('education').select('program');
...
router.get('/search', authenticate, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    if (program || batch) {
      let eduQuery = db.from('education').select('profile_id');
      if (program) {
    ...
    if (company) {
      let empQuery = db.from('employment').select('profile_id')
        .eq('is_current', true)
        .ilike('company_name', `%${sanitizeFilterInput(company)}%`);
    ...
    if (q) {
      const sanitized = sanitizeFilterInput(q);
    ...
    if (location) {
      const sanitizedLocation = sanitizeFilterInput(location);
      profileQuery = profileQuery.or(
        `city.ilike.%${sanitizedLocation}%,province.ilike.%${sanitizedLocation}%`
      );
    }
    ...
    if (profileIds.length > 0) {
      const [{ data: employment }, { data: education }] = await Promise.all([
        db.from('employment').select('profile_id, company_name, position, employment_status, is_current')
          .in('profile_id', profileIds).eq('is_current', true),
        db.from('education').select('profile_id, program, year_graduated, honors')
...
router.get('/:id', authenticate, async (req: AuthenticatedRequest, res, next) => {
  try {
    ...
    const [achievementsRes, feedbackRes] = await Promise.all([
      db.from('achievements').select('*').eq('profile_id', req.params.id).order('date_achieved', { ascending: false }),
      db.from('career_feedback').select('*').eq('profile_id', req.params.id).maybeSingle(),
    ]);
    ...
    if (currentProfile) {
      const sanitizedId = sanitizeFilterInput(req.params.id);
      const { data: connectionStatus } = await db
        .from('connections')
        .select('id, status')
        .or(`and(requester_id.eq.${currentProfile.id},recipient_id.eq.${sanitizedId}),and(requester_id.eq.${sanitizedId},recipient_id.eq.${currentProfile.id})`)
        .maybeSingle();
