...
export default function ReportsPage() {
  ...
  const jsonToCsv = (data: any, filename: string, reportId: string, suffix: string) =>
    handleExport(
  ...
  const reports = [
    ...
    },
    {
      ...
      desc: 'List of registered companies, their industry, and location details.',
    ...
    },
    {
      ...
      desc: 'Distribution of reported salaries by monthly salary bracket.',
      exportCsv: () => adminApi.salaryDistribution().then((d: any) => jsonToCsv(d, 'salary-distribution.csv', 'salary', 'csv')),
      exportJson: () => handleExport(() => adminApi.salaryDistribution().then((d: any) => new Blob([JSON.stringify(d, null, 2)], { type: 'application/json' })), 'salary-distribution.json', 'salary-json'),
    },
    {
      id: 'employment-rate',
      title: 'Employment Rate Report',
      desc: 'Employment rate breakdown by course.',
      exportCsv: () => adminApi.employmentByCourse().then((d: any) => jsonToCsv(d, 'employment-rate-by-course.csv', 'erate', 'csv')),
      exportJson: () => handleExport(() => adminApi.employmentByCourse().then((d: any) => new Blob([JSON.stringify(d, null, 2)], { type: 'application/json' })), 'employment-rate.json', 'erate-json'),
    },
    {
      id: 'curriculum',
      title: 'Degree Alignment Report',
      desc: 'Degree-to-employment alignment rate by course.',
      exportCsv: () => adminApi.degreeAlignment().then((d: any) => jsonToCsv(d, 'degree-alignment.csv', 'curriculum', 'csv')),
      exportJson: () => handleExport(() => adminApi.degreeAlignment().then((d: any) => new Blob([JSON.stringify(d, null, 2)], { type: 'application/json' })), 'degree-alignment.json', 'curriculum-json'),
    },
    {
    ...
    },
    {
      ...
      exportCsv: () => adminApi.employmentByBatch().then((d: any) => jsonToCsv(d, 'batch-employment.csv', 'batch', 'csv')),
