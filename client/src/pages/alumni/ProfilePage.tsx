...
function calcProfileCompletion(profile: any): number {
  ...
  const sections = [
    ...
    !!(profile.current_job_title || profile.company_name || profile.employment_status),
  ];
  const score = filled + sections.filter(Boolean).length;
  return Math.min(100, Math.round((score / (baseFields.length + sections.length)) * 100));
}
  ...
  const profileStats = [
    {
      ...
      value: profile?.employment_status?.toLowerCase() === 'student' ? 'Unemployed' : (profile?.employment_status || 'Not set'),
      icon: BriefcaseIcon,
    },
    {
            ...
            {currentEmp || profile?.employment_status ? (
              <div className="space-y-2 text-sm">
                <div>
                  ...
                  <span className="font-medium text-gray-800">{((status) => status?.toLowerCase() === 'student' ? 'Unemployed' : status)(profile?.employment_status || currentEmp?.employment_status || 'Employed')}</span>
                </div>
